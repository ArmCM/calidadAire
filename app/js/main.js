// Main js file with AMD (before main.js and mod_mapa.js)
// ------------------------------------------------------------

define(
  ['d3', 'leaflet', 'chart', 'stations', 'states', 'common'],
  (d3, L, Chart, stations, states, common) => {
  var dataLocal = [];
  var chart;
  var ctx;
  var color;
  var mymap = common.get_mymap();
  ...
  $(document).ready(function () {
    ...
    for (var i = 0; i < stations.estaciones_json.length; i++) {
      //v 2.0 validacion de la actividad de la estación
      if (stations.estaciones_json[i].activa) {
        marker_mymap = L.marker([stations.estaciones_json[i].lat, stations.estaciones_json[i].long], { icon: greenIcon }).addTo(myFeatureGroup).bindPopup("Cargando...");;
        marker_mymap.idestacion = stations.estaciones_json[i].id;
  ...
  function groupClick(event) {
    ...
    var info = "<b>Estación:</b> " + stations.estaciones_json[array].id + "<br><b>Nombre: </b>" + stations.estaciones_json[array].nombre + "<br><b>Codigo: </b>" + stations.estaciones_json[array].codigo + '<br><div style="margin-bottom: 25px; margin-top: 25px;" class="botonera"><a class="modal_mapa" data-id=""+ stations.estaciones_json[array].id +"">Detalle Estación</a></div>';
  ...
  function buscarCiudad(idEstacion) {
    var city = "";
    for (let index = 0; index < stations.estaciones_json.length; index++) {
      const element = stations.estaciones_json[index];
  ...
  function options_estado() {
    ...
    for (let index = 0; index < states.coor_estado.length; index++) {
      const element = states.coor_estado[index];
  ...
  function putGrafica(parametro, horas, promedio2, maximo) {
    ...
    var labels_temp2 = [];
    for (var i = 0; i < labels_temp.length; i++) {
      labels_temp2[i] = common.get_fecha_formato(labels_temp[i]);
    }
  ...
  function getFormatDateAPI(d) {
    var fecha = d.getFullYear() + "-" + common.meis[d.getMonth()] + "-" + ((d.getDate() < 10 ? "0" : "") + d.getDate()) + "T" + ((d.getHours() < 10 ? "0" : "") + d.getHours()) + ":" + ((d.getMinutes() < 10 ? "0" : "") + d.getMinutes()) + ":00";
    return fecha;
  }
  ...
  function ponEstacionesSel() {
    ...
    var numEstaciones = 0;
    for (let index = 0; index < stations.estaciones_json.length; index++) {
      var element = stations.estaciones_json[index];
  ...
  function cambiaCoor() {
    ...
    jQuery.each(states.coor_estado, function (i, val) {
